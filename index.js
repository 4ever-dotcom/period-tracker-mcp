...
function createServer() {
  ...
  server.tool(
    'add_period',
    '记录一次经期，日期格式YYYY-MM-DD',
    {
      start_date: z.string().describe('经期开始日期，如 2026-04-11'),
      notes: z.string().optional().describe('备注，如 身体不适等症状'),
    },
    async ({ start_date, notes }) => {
      // 验证日期格式
      const date = new Date(start_date);
      if (isNaN(date.getTime())) {
        return { content: [{ type: 'text', text: '日期格式不对，请使用YYYY-MM-DD格式，比如2026-04-11' }] };
      }
      ...
      if (all.length >= 2) {
        ...
        return { 
          content: [{ 
            type: 'text', 
            text: `已记录。${formatDate(date)}这次经期已保存。\n\n平均周期${avg}天，下一次大概在${formatDate(nextDate)}左右。` 
          }] 
        };
      }
      
      return { content: [{ type: 'text', text: `已记录。${formatDate(date)}这次经期已保存。再记录一次即可计算周期。` }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ limit }) => {
      ...
      if (!records.length) {
        return { content: [{ type: 'text', text: '暂无经期记录。' }] };
      }
  ...
  server.tool(
    'get_cycle_status',
    '查看当前处于周期的哪个阶段，预测下次大概时间，以便提前准备',
    {},
    async () => {
      const records = await sbGet('period_records', 'select=*&order=start_date.desc');
      
      if (!records.length) {
        return { content: [{ type: 'text', text: '暂无经期记录，无法预测。' }] };
      }
      ...
      let status = '';
      if (daysUntilNext < 0) {
        status = `已延期${-daysUntilNext}天，可能即将来临`;
      } else if (daysUntilNext === 0) {
        status = '预计今天开始';
      } else if (daysUntilNext <= 5) {
        status = `还有${daysUntilNext}天，请提前做好准备`;
      } else {
  ...
  server.tool(
    'add_ovulation',
    '记录一次排卵期，日期格式YYYY-MM-DD',
    {
      ovulation_date: z.string().describe('排卵期开始日期，如 2026-05-25'),
      notes: z.string().optional().describe('备注，如 身体特征变化'),
    },
    async ({ ovulation_date, notes }) => {
      const date = new Date(ovulation_date);
      if (isNaN(date.getTime())) {
        return { content: [{ type: 'text', text: '日期格式不对，请使用YYYY-MM-DD格式，比如2026-05-25' }] };
      }
      
      await sbPost('ovulation_records', { ovulation_date, notes: notes || '' });
      
      return { content: [{ type: 'text', text: `已记录。${formatDate(date)}这次排卵期已保存。` }] };
    }
  );
  ...
  server.tool(
    ...
    async ({ limit }) => {
      ...
      if (!records.length) {
        return { content: [{ type: 'text', text: '暂无排卵期记录。' }] };
      }
